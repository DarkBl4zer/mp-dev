...
function CargarDatos() {
    _POST('json', '/juzgados/parte/actuaciones', {}, function (result){
        if (result.length > 0) {
            LlenaTabla(result);
        } else {
...
function NuevaActuacion() {
    _POST('html', '/juzgados/parte/modal', {}, function (result){
...
function Ver(id) {
    var datos = {id: id};
    _POST('html', '/juzgados/parte/modal_ver', JSON.stringify(datos), function (result){
        $('#espacioModal').html(result);
    });
}
...
function RegistrarActuacion(){
    ...
    var campos = ['tipo_actuacion','clase_diligencia','despacho_judicial','numero','clase','fecha_actuacion','observaciones','archivo','txtSinproc'];
