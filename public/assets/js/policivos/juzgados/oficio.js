...
function CargarDatos() {
    _POST('json', '/juzgados/oficio/actuaciones', {}, function (result){
        if (result.length > 0) {
            LlenaTabla(result);
        } else {
...
function LlenaTabla(datos) {
    ...
    $('#tablaDatos').DataTable({
        columns: [
            ...
            { title: "Clase de proceso" },
            { title: "NÚMERO PROCESO" },
            { title: "Detalle" }
        ],
        columnDefs: [
...
function NuevaActuacion() {
    clicksMas = 0;
    _POST('html', '/juzgados/oficio/modal', {}, function (result){
...
function Ver(id) {
    clicksMas = 0;
    var datos = {id: id};
    _POST('html', '/juzgados/oficio/modal_ver', JSON.stringify(datos), function (result){
        $('#espacioModal').html(result);
    });
}
...
function CambioTipoActuacion(ta) {
    var datos = {ta: ta};
    _POST('json', '/juzgados/oficio/cambio_tipo_actuacion', JSON.stringify(datos), function (result){
...
function RegistrarActuacion(){
    ...
    var campos = ['tipo_actuacion','clase_diligencia','despacho_judicial','numero','clase','fecha_actuacion','observaciones'];
