...
function CargarDatos() {
    _POST('json', '/movilidad/parte/actuaciones', {}, function (result){
        if (result.length > 0) {
            LlenaTabla(result);
        } else {
...
function NuevaActuacion() {
    _POST('html', '/movilidad/parte/modal', {}, function (result){
...
function Ver(id) {
    var datos = {id: id};
    _POST('html', '/movilidad/parte/modal_ver', JSON.stringify(datos), function (result){
        $('#espacioModal').html(result);
    });
}
...
function RegistrarActuacion(){
    ...
    var campos = ['tipo_actuacion','clase_diligencia','clase','numero','fecha_actuacion','observaciones','archivo','txtSinproc'];
