...
function CargarDatos() {
    _POST('json', '/penales1/parte/actuaciones', {}, function (result){
        if (result.length > 0) {
            LlenaTabla(result);
        } else {
...
function NuevaActuacion() {
    _POST('html', '/penales1/parte/modal', {}, function (result){
...
function Ver(id) {
    var datos = {id: id};
    _POST('html', '/penales1/parte/modal_ver', JSON.stringify(datos), function (result){
        $('#espacioModal').html(result);
    });
}
...
function RegistrarActuacionSinproc(){
    ...
    if (valida) {
        ...
        _POST('html', '/penales1/parte/guardar_act_sinproc', JSON.stringify(datos), function (result){
