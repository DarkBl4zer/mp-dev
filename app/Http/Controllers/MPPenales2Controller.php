...
class MPPenales2Controller extends Controller
{
    ...
    //================================= De Oficio =================================
        public function oficio(){
            if ($this->estoy_logeado()) {
                ...
                return view('penales2.oficio', compact('festivos'));
            } else {
        ...
        public function oficio_actuaciones(Request $request){
            ...
            $retorno = Data_Penales_Dos::where('eliminado', false)->where('tipo_formulario', 1)->where('usuario_crea', $cedula)->get();
        ...
        public function oficio_modal(Request $request){
            $parametricas['actuacion'] = $this->GenerarSelect(Para_Actuacion::where('eliminado', false)->whereIn('id_tipo_mp', [2,6])->orderBy('nombre', 'desc')->get());
            $parametricas['criterios'] = $this->GenerarSelect(Para_Criterio_Intervencion::where('eliminado', false)->get());
            $parametricas['despacho'] = $this->GenerarSelect(Para_Despacho::where('eliminado', false)->whereIn('id_tipo_mp', [2,6])->get(), true);
            ...
            return view('penales2.oficio_crear', compact('parametricas'));
        }
        ...
        public function cambio_tipo_actuacion(Request $request){
            ...
            $estado_audiencia = Para_Estado_Audiencia::where('eliminado', false)->whereIn('id_tipo_mp', [2,6])->get();
        ...
        public function oficio_guardar(Request $request){
            ...
            $idP1 = Data_Penales_Dos::create($campos)->id;
            ...
            return redirect('penales2/oficio')->with('success', 'Registro de Penales dos (Oficio) creado correctamente en el sistema con el id: ' . $idP1 . '.');
        }

        public function oficio_modal_ver(Request $request){
            $consulta = $request->all();
            $campos = Data_Penales_Dos::where('id', $consulta['id'])->first();
            ...
            $denunciados = Data_Denunciados::where('id_tipo_mp', 2)->where('id_actuacion', $consulta['id'])->get();
            ...
            return view('penales2.oficio_ver', compact('campos', 'rdenunciados'));
        }

        public function archivo($archivo){
            $archivo_or = Data_Penales_Dos::where('archivo_dt', $archivo)->first()->archivo_or;
            $ptArchivo = storage_path(). '/penales2/' . $archivo;
    ...
    //================================= De Parte =================================
        public function parte(){
            if ($this->estoy_logeado()) {
                $festivos = $this->Festivos();
                return view('penales2.parte', compact('festivos'));
            } else {
                return redirect('login');
            }
        }
        ...
        public function parte_actuaciones(Request $request){
            $sesion = Session::get('UsuarioMp');
            $cedula = $sesion[0]['cedula'];
            $retorno = Data_Penales_Dos::where('eliminado', false)->where('tipo_formulario', 2)->where('usuario_crea', $cedula)->get();
            for ($i=0; $i < count($retorno); $i++) { 
                $retorno[$i]['txt_tipo_actuacion'] = str_replace("0_", "", Para_Actuacion::where('valor', $retorno[$i]['tipo_actuacion'])->first()->nombre);
                $retorno[$i]['txt_clase_diligencia'] = Para_Clase_Diligencia::where('valor', $retorno[$i]['clase_diligencia'])->first()->nombre;
                $retorno[$i]['txt_estado_audiencia'] = ( $retorno[$i]['estado_audiencia'] === null) ? "":Para_Estado_Audiencia::where('valor', $retorno[$i]['estado_audiencia'])->first()->nombre;
                ...
                if ($this->ChecarSinproc($cedula, $retorno[$i]['sinproc'])) {
                    $cont = Data_Penales_Dos::where('eliminado', false)->where('sinproc', $retorno[$i]['sinproc'])->where('habilitar_archivo', true)->count();
                    if ($cont > 0) {
        ...
        public function parte_modal(Request $request){
            ...
            return view('penales2.parte_crear', compact('parametricas', 'sinprocs'));
        }
        ...
        public function parte_guardar(Request $request){
            ...
            $idP1 = Data_Penales_Dos::create($campos)->id;
            return redirect('penales2/parte')->with('success', 'Registro de Penales dos (Parte) creado correctamente en el sistema con el id: ' . $idP1 . '.');
        }
        ...
        public function parte_modal_ver(Request $request){
            ...
            return view('penales2.parte_ver', compact('campos', 'datos_sinproc'));
        }
        ...
        public function parte_guardar_act_sinproc(Request $request){
            ...
            $parametros = $request->all();
            $id = Data_Penales_Dos::where('eliminado', false)->where('sinproc', $parametros['sinproc'])->first()->id;
            $arr = array(
                'habilitar_archivo' => true,
                'usuario_modifica' => $cc
            );
            Data_Penales_Dos::find($id)->update($arr);
            return 'true';
        }
    ...
    //================================= Enteramientos =================================
        public function enteramientos(){
            if ($this->estoy_logeado()) {
                ...
                $festivos = $this->Festivos();
                return view('penales2.enteramientos', compact('festivos', 'rol'));
            } else {
                return redirect('login');
            }
        }
        ...
        public function enteramientos_listas(Request $request){
            ...
            $rol = $sesion[0]['rol'];
            if ($rol == 7) {
                $retorno = Data_Enteramiento::where('eliminado', false)->where('id_tipo_mp', 2)->get();
            } else {
                $retorno = Data_Enteramiento::where('eliminado', false)->where('id_tipo_mp', 2)->where('usuario_crea', $cedula)->get();
            }
            for ($i=0; $i < count($retorno); $i++) { 
                ...
                if ($retorno[$i]['archivo_dt'] != '') {
                    $retorno[$i]['btn_acciones'] = '<a  href="/penales2/archivo_enteramientos/'.$retorno[$i]['archivo_dt'].'" class="btn btn-primary"><i class="fas fa-paperclip"></i></a>';
                } else {
        ...
        public function enteramientos_modal(Request $request){
            $parametricas['unidad'] = $this->GenerarSelect(Para_Unidad::where('eliminado', false)->whereIn('id_tipo_mp', [2,6])->get());
            return view('penales2.enteramientos_crear', compact('parametricas'));
        }
        ...
        public function enteramientos_guardar(Request $request){
            ...
            return redirect('penales2/enteramientos')->with('success', 'Registro de Penales dos (Enteramientos) creado correctamente en el sistema con el id: ' . $idP1 . '.');
        }
    ...
    //================================= Agencias =================================
        public function agencias(){
            if ($this->estoy_logeado()) {
                ...
                $festivos = $this->Festivos();
                return view('penales2.agencias', compact('noti', 'festivos'));
            } else {
                return redirect('login');
            }
        }
        ...
        public function agencias_tabla(){
            $sesion = Session::get('UsuarioMp');
            $cedula = $sesion[0]['cedula'];
            $retorno = Data_Agencia_Especial::where('eliminado', false)->where('estado', '!=', 4)->where('nombre_ministerio', $cedula)->get();
            for ($i=0; $i < count($retorno); $i++) { 
        ...
        public function agencias_intervencion(Request $request){
            ...
            $parametricas['actuacion'] = $this->GenerarSelect(Para_Actuacion::where('eliminado', false)->whereIn('id_tipo_mp', [2,6])->orderBy('nombre', 'desc')->get());
            $parametricas['criterios'] = $this->GenerarSelect(Para_Criterio_Intervencion::where('eliminado', false)->get());
            $parametricas['despacho'] = $this->GenerarSelect(Para_Despacho::where('eliminado', false)->whereIn('id_tipo_mp', [2,6])->get(), true, $agencia->despacho_judicial);
            $parametricas['delito'] = $this->GenerarSelect(Para_Delito::where('eliminado', false)->get(), true, $agencia->adecuacion_tipica);
            $parametricas['sexo'] = $this->ComboSexo();
            $parametricas['identidad'] = $this->ComboIdentidad();
            $parametricas['orientacion'] = $this->ComboOrientacion();
            $parametricas['nacionalidad'] = $this->ComboPaises();
            return view('penales2.agencias_intervencion', compact('parametricas', 'idInt', 'agencia'));
        }
        ...
        public function agencias_intervencion_guardar(Request $request){
            ...
            return redirect('penales2/agencias')->with('success', 'Registro de Agencias especiales (Intervención) creado correctamente en el sistema con el id: ' . $idP1 . '.');
        }
        ...
        public function agencias_intervenciones(Request $request){
            ...
            $sesion = Session::get('UsuarioMp');
            $cedula = $sesion[0]['cedula'];
            $retorno = Data_Penales_Dos::where('eliminado', false)->where('tipo_formulario', 4)->where('agencia_especial', $idInt)->get();
            for ($i=0; $i < count($retorno); $i++) { 
                $retorno[$i]['txt_tipo_actuacion'] = str_replace("0_", "", Para_Actuacion::where('valor', $retorno[$i]['tipo_actuacion'])->first()->nombre);
                $retorno[$i]['txt_clase_diligencia'] = Para_Clase_Diligencia::where('valor', $retorno[$i]['clase_diligencia'])->first()->nombre;
                $retorno[$i]['txt_estado_audiencia'] = ( $retorno[$i]['estado_audiencia'] === null) ? "":Para_Estado_Audiencia::where('valor', $retorno[$i]['estado_audiencia'])->first()->nombre;
            ...
            return view('penales2.agencias_intervenciones', compact('retorno', 'idInt'));
        }
        ...
        public function agencias_informe(Request $request){
            ...
            return view('penales2.agencias_informe', compact('agencia', 'idInt', 'parametricas', 'ultimo_informe', 'denunciados'));
        }
        ...
        public function agencias_informe_guardar(Request $request){
            ...
            return redirect('penales2/agencias')->with('success', 'Registro de Informe de Agencia Especial #'.$campos['numero_agencia_especial'].' creado correctamente en el sistema con el id: ' . $idP1 . '.');
        }
        ...
        public function agencias_informes(Request $request){
            ...
            return view('penales2.agencias_informes', compact('agencia', 'informes', 'idAE'));
        }
        ...
        public function agencias_informe_ver(Request $request){
            ...
            return view('penales2.agencias_informe_ver', compact('informe', 'agencia', 'rdenunciados'));
        }
}
