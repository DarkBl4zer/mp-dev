...
@section('modalTitle')
NUEVA INTERVENCIÓN AGENCIA ESPECIAL #{{sprintf("%03d", $idInt)}}
@endsection
@section('modalBtnsHeader')
<button type="button" class="btn btn-secondary" onclick="VerIntervenciones({{$idInt}});" data-toggle="tooltip" data-html="true" title="Intervenciones realizadas"><i class="fas fa-list-ol"></i></button>
@endsection
@section('modalBody')
<form action="{{route('penales2_agencias_intervencion_guardar')}}" id="formularioOficio" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <input type="hidden" name="agencia_especial" value="{{$idInt}}">
    ...
    <div class="row top10">
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" id="noticia_criminal" name="noticia_criminal" autocomplete="off" maxlength="21" minlength="21" onkeyup="this.value = FiltrarCaracteres(this, 'numeros');$('#M' + this.id).html('').hide();" onblur="ValidarMinimo(this.id, 21);" value="{{$agencia->noticia_criminal}}"/>
